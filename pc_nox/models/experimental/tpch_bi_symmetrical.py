...
Equinox implementation of temporal Hierarchical Predictive Coding (tPC-H), combined with Bidirectional Predictive Coding.
...
class TpchControlLayer(eqx.Module):
    ...
    W_child_curr: eqx.nn.Linear # discriminative bottom up weight
    W_child_prev: eqx.nn.Linear # discriminative bottom up weight
    ...
    def __init__(
        ...
        *,
        ...
        key_rec, key_in, key_child_curr, key_child_prev = jr.split(key, 4)
        ...
        self.W_child_curr = eqx.nn.Linear(child_size, state_size, use_bias=False, key=key_child_curr)
        self.W_child_prev = eqx.nn.Linear(child_size, state_size, use_bias=False, key=key_child_prev)
        self.act_fn = act_fn
    ...
    def predict(self, state_prev: Array, child_curr: Array, child_prev: Array, control_input: Optional[Array] = None) -> Array:
        """s_hat_t = f(A @ s_{t-1} + B @ x_t)"""
        rec_term = self.W_rec(state_prev)
        rec_term += + self.W_child_curr(child_curr)
        rec_term += + self.W_child_prev(child_prev)
...
class TpchHiddenLayer(eqx.Module):
    ...
    W_child_prev: eqx.nn.Linear # discriminative bottom up weight
    act_fn: Callable = eqx.field(static=True)
    lowest_hidden: bool = eqx.field(static=True)
    ...
    def __init__(
        ...
        *,
        key: PRNGKeyArray,
        lowest_hidden: bool = False  # whether this is the lowest (closest to observation) hidden layer, relevant for symmetric bidirection
    ):
        key_rec, key_parent_prev, key_parent_curr, key_child_curr, key_child_prev = jr.split(key, 5)
        self.W_rec = eqx.nn.Linear(state_size, state_size, use_bias=False, key=key_rec)
        ...
        self.W_child_curr = eqx.nn.Linear(child_size, state_size, use_bias=False, key=key_child_curr)
        if lowest_hidden:
            self.lowest_hidden = True
            self.W_child_prev = None
        else:
            self.lowest_hidden = False
            self.W_child_prev = eqx.nn.Linear(child_size, state_size, use_bias=False, key=key_child_prev)
        self.act_fn = act_fn

    def predict(self, state_prev: Array, parent_prev: Array, parent_curr: Array, child_curr: Array, child_prev: Array) -> Array:
        """z_hat_t = f(P @ z_{t-1} + Q @ s_{t-1} + R @ s_t)"""
        pre_acts = (self.W_rec(state_prev)
            + self.W_parent_prev(parent_prev)
            + self.W_parent_curr(parent_curr)
            + self.W_child_curr(child_curr))
        if not self.lowest_hidden:
            pre_acts += self.W_child_prev(child_prev)
...
class TpchModel(eqx.Module, ModelBase):
    ...
    def __init__(
        ...
        self.control_layer = TpchControlLayer(
            state_size=control_layer_size, child_size=hidden_sizes[0], input_size=input_size, act_fn=act_fn_callable, key=key_control
        )
        # Create a list of child sizes by shifting hidden_sizes left and appending the final layer's child size
        child_sizes = list(hidden_sizes[1:]) + [obs_size]

        hidden_layers = []
        parent_size = control_layer_size

        for size, child_size, hkey in zip(hidden_sizes[:-1], child_sizes[:-1], hidden_keys[:-1]):
        ...
        hidden_layers.append(
                TpchHiddenLayer(
                    state_size=hidden_sizes[-1],
                    parent_size=parent_size,
                    child_size=child_sizes[-1],
                    act_fn=act_fn_callable,
                    key=hidden_keys[-1],
                    lowest_hidden=True,
                )
            )
        parent_size = hidden_sizes[-1]
    ...
    def predict(
        ...
        """Run every layer's combined prediction once.

        `observation` is needed for the lowest hidden layer's discriminative
        bottom-up prediction, because its child at the current timestep is
        the actual observation `y_t`.
        """
        if self.hidden_layers and observation is None:
            raise ValueError(
                "`observation` is required by predict() when bottom-up "
                "weights are present."
            )

        predictions = [
            self.control_layer.predict(
                states_prev[0],
                child_curr=states_curr[1],
                child_prev=states_prev[1],
        ...
        for i, layer in enumerate(self.hidden_layers):
            ...
            child_prev = (
                jnp.zeros_like(observation) if i == len(self.hidden_layers) - 1
                else states_prev[i + 2]
            )
            predictions.append(
                layer.predict(
                    own_prev,
    ...
    def init_activities(
        ...
        """Bottom-up amortised "kick-start" pass: produces an initial prediction 
        sweeping bottom-to-top starting from observation `y`.
        """
        child_pred = y
        hidden_preds_bottom_up = []

        # Sweep bottom-to-top through hidden layers.
        for i in reversed(range(len(self.hidden_layers))):
            layer: TpchHiddenLayer = self.hidden_layers[i]
            own_prev = states_prev[i + 1]
            parent_prev = jnp.zeros_like(states_prev[i])
            parent_curr = jnp.zeros_like(states_prev[i])
            
            if layer.lowest_hidden:
                child_prev = jnp.zeros_like(child_pred)
            else:
                child_prev = states_prev[i + 2]

            prediction = layer.predict(
                state_prev=own_prev,
                parent_prev=parent_prev,
                parent_curr=parent_curr,
                child_curr=child_pred,
                child_prev=child_prev
            )
            hidden_preds_bottom_up.append(prediction)
            child_pred = prediction  # Pass prediction up to the next layer

        # Process the top control layer using the prediction from the top hidden layer.
        control_pred = self.control_layer.predict(
            states_prev[0], 
            child_pred,
            states_prev[1],
            control_input
        ...
        # 4. Reconstruct top-down order for downstream compatibility:
        # [control_pred, hidden_0, hidden_1, ..., hidden_bottom]
        hidden_preds_top_down = list(reversed(hidden_preds_bottom_up))
        states_curr = [control_pred] + hidden_preds_top_down

        return states_curr


    def init_activities_top_down(
        self, 
        states_prev: Activities,
        y: Array, # used for its size only, to generate zeros
        control_input: Optional[Array] = None
    ) -> Activities:
        """Feedforward "purely generative pass: produces an initial prediction for every
        ...
        """
        child_curr = jnp.zeros_like(states_prev[1])
        control_pred = self.control_layer.predict(states_prev[0], child_curr, states_prev[1], control_input)
        ...
        for i, layer in enumerate(self.hidden_layers[:-1]):
            ...
            parent_prev = states_prev[i]
            child_curr = jnp.zeros_like(states_prev[i + 2])
            child_prev = jnp.zeros_like(states_prev[i + 2])
            prediction = layer.predict(own_prev, parent_prev, parent_pred, child_curr, child_prev)
            states_curr.append(prediction)
            parent_pred = prediction

        # 3. Last hidden layer
        last_layer = self.hidden_layers[-1]
        own_prev = states_prev[-1]
        parent_prev = states_prev[-2]
        child_curr = jnp.zeros_like(y)
        child_prev = jnp.zeros_like(y)
        
        prediction = last_layer.predict(own_prev, parent_prev, parent_pred, child_curr, child_prev)
        states_curr.append(prediction)
    ...
    def _ff_weights(self) -> List[Array]:
        ...
        weights.append(self.control_layer.W_child_curr.weight)
        for layer in self.hidden_layers:
            weights += [
                layer.W_parent_prev.weight,
                layer.W_parent_curr.weight,
                layer.W_child_curr.weight,
            ]
    ...
    def _weight_entries(self) -> List[Tuple[int, bool, Array]]:
        ...
        entries.append((0, False, self.control_layer.W_child_curr.weight))
        for i, layer in enumerate(self.hidden_layers, start=1):
            entries += [
                ...
                (i, False, layer.W_child_curr.weight),
            ]
    ...
    def tpch_energy_fn(
        ...
        forward_energy_weight: Union[float, Array] = 1.0,
        backward_energy_weight: Union[float, Array] = 1.0,
    ) -> Array:
        """
        Sum the bidirectional tPC-H energy over all latent layers.

        For each latent layer, the energy contains two independent
        prediction-error terms:

            1/2 * forward_energy_weight
                * ||state_t - forward_prediction_t||^2

            1/2 * backward_energy_weight
                * ||state_t - backward_prediction_t||^2

        The forward prediction is the original temporal tPC-H prediction:
        it retains the recurrent state and the parent's previous/current
        states. The backward prediction is supplied by that layer's
        `W_child_curr` discriminative pathway.

        For the lowest hidden layer, the backward predictor receives the
        current observation `y_t`, matching the bottom-up endpoint in the
        bidirectional PC formulation.

        The observation/output reconstruction term remains part of the
        forward/generative pathway and is therefore multiplied by
        `forward_energy_weight`.

        `weight_reg_total` and `return_layerwise` retain their previous
        semantics. In particular, `return_layerwise=True` still returns one
        combined energy per latent layer, followed by the observation term,
        so existing plotting code does not need to change.
        """
        # ------------------------------------------------------------------
        # Forward/temporal predictions.
        #
        # These deliberately exclude W_child_curr. That weight belongs to
        # the separate bottom-up error term below.
        # ------------------------------------------------------------------
        forward_predictions = []

        # Control layer: recurrent state + optional current control input.
        control_forward_pre = self.control_layer.W_rec(states_prev[0])
        if self.control_layer.has_input and control_input is not None:
            control_forward_pre = (
                control_forward_pre
                + self.control_layer.W_in(control_input)
            )
        forward_predictions.append(
            self.control_layer.act_fn(control_forward_pre)
        )

        # ------------------------------------------------------------------
        # Backward/discriminative predictions.
        # ------------------------------------------------------------------
        backward_predictions = [
            self.control_layer.W_child_curr(states_curr[1])
        ]

        # Hidden layers: preserve the complete temporal/top-down pathway in
        # the forward prediction, while the child pathway is kept separate.
        for i, layer in enumerate(self.hidden_layers):
            own_prev = states_prev[i + 1]
            parent_prev = states_prev[i]
            parent_curr = states_curr[i]
            ...
            forward_pre = (
                layer.W_rec(own_prev)
                + layer.W_parent_prev(parent_prev)
                + layer.W_parent_curr(parent_curr)
            )
            forward_predictions.append(layer.act_fn(forward_pre))

            child_curr = (
                observation if i == len(self.hidden_layers) - 1
                else states_curr[i + 2]
            )
            if not layer.lowest_hidden:
                child_curr = states_curr[i + 2]
                child_prev = states_prev[i + 2]
                backward_pre = layer.W_child_curr(child_curr) + layer.W_child_prev(child_prev)
            else:
                child_curr = observation
                backward_pre = layer.W_child_curr(child_curr)

            backward_predictions.append(layer.act_fn(backward_pre))

        # Each latent layer receives one forward and one backward error.
        layer_energies = []
        for state, forward_prediction, backward_prediction in zip(
            states_curr,
            forward_predictions,
            backward_predictions,
        ):
            forward_error = state - forward_prediction
            backward_error = state - backward_prediction

            layer_energies.append(
                0.5 * forward_energy_weight * jnp.sum(forward_error ** 2)
                + 0.5 * backward_energy_weight * jnp.sum(backward_error ** 2)
            )

        # The observation layer remains a forward/generative reconstruction.
        y_hat = self.observation_layer.predict(states_curr[-1])
        if self.config.loss == "mse":
            ...
            obs_energy = (
                forward_energy_weight
                * -jnp.sum(observation * jax.nn.log_softmax(y_hat))
            )
        ...
        if return_layerwise:
            ...
            # the weight-reg breakdowns and layer_energies.
            activity_by_layer = jnp.concatenate(
                [
                    activity_by_layer,
                    jnp.zeros((1,), dtype=activity_by_layer.dtype),
                ]
            )
            return (
                jnp.stack(layer_energies)
                + weight_l2_by_layer
                + weight_orth_by_layer
                + activity_by_layer
            )
        ...
        return (
            sum(layer_energies)
            + weight_reg_total
            + self._activity_reg(states_curr)
        )
    ...
    def settle(
        ...
        states_curr = self.init_activities(observation, states_prev, control_input)
    ...
    def settle_scan(
        ...
        """
        states_curr0 = self.init_activities(observation, states_prev, control_input)
...
def make_eval_step(activity_optim: optax.GradientTransformation, n_infer_steps: int, control_input: Optional[Array] = None):
    ...
    def eval_step(model: TpchModel, states_prev, y, return_layerwise: bool = False):
        states_curr_init = model.init_activities(y, states_prev, control_input)
...
def make_train_step(param_optim: optax.GradientTransformation, activity_optim: optax.GradientTransformation, n_infer_steps: int, control_input: Optional[Array] = None):
    ...
    def train_step(model: TpchModel, param_opt_state, states_prev, y, return_layerwise: bool = False):
        ...
        # TODO revert energy_before to full discrim states_curr_init
        energy_before = model.tpch_energy_fn(states_prev, states_curr_init, y, control_input)
